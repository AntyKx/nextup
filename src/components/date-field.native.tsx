...
export function DateField({ dueDate, onChange }: { dueDate: string | null; onChange: (iso: string) => void }) {
  ...
  // No date chosen yet — the picker still needs a starting point to open on, but the displayed text stays a placeholder until the user actually picks something.
  const pickerValue = parseLocalDate(dueDate ?? formatIsoDate(new Date()));
    ...
    <View>
      <Pressable onPress={open} style={styles.dateRow}>
        <View style={styles.dateInput}>
          <Text style={dueDate ? styles.dateInputText : styles.dateInputPlaceholder}>{dueDate ?? '請選擇實際日期'}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  dateInputPlaceholder: { color: palette.subtle, fontSize: 15, fontFamily: fonts.bodyMedium, letterSpacing: 0.4 },
